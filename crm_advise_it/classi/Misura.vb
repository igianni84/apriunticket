...
#Region "Private Method"
    ...
    Private Sub _load(ByVal _criterio As Integer)
        Dim SQL As String = "SELECT * FROM Misura WHERE ID=" & _criterio

        Dim DTR As SqlClient.SqlDataReader = Me.Mygest.GetReader(SQL)

        If DTR.Read Then
            Me.ID = DTR("ID")
            Me.Misura = DTR("misura")
...
#Region "Public Method"
    ...
    Public Function SalvaData() As Boolean
        ...
        pr.Parameters.AddWithValue("@misura", Me.Misura)
        pr.Parameters.AddWithValue("@idazienda", Me.Azienda.ID)
        Dim tb As DataTable = Me.Mygest.GetTab("IMisura", pr.Parameters)
    ...
    Public Function Delete(ByVal criterio As Integer) As Boolean
        Dim ris As Boolean = False
        Dim SQL As String = "DELETE FROM Misura WHERE ID=" & criterio
        Try
