...
#Region "Private Method"
    ...
    Private Sub _load(ByVal _criterio As Integer)
        Dim SQL As String = "SELECT * FROM TipoUtente WHERE ID=" & _criterio

        Dim DTR As SqlClient.SqlDataReader = Me.Mygest.GetReader(SQL)

        If DTR.Read Then
            Me.ID = DTR("ID")
            Me.TipoUtente = DTR("tipoutente")
        End If
        DTR.Close()
    End Sub
...
#Region "Public Method"
    ...
    Public Function SalvaData() As Boolean
        ...
        pr.Parameters.AddWithValue("@tipoutente", Me.TipoUtente)
        Dim tb As DataTable = Me.Mygest.GetTab("ITipoUtente", pr.Parameters)
    ...
    Public Function Delete(ByVal criterio As Integer) As Boolean
        Dim ris As Boolean = False
        Dim SQL As String = "DELETE FROM TipoUtente WHERE ID=" & criterio
        Try
